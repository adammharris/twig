//! Generated from the WHATWG HTML5 named character reference table
//! (https://html.spec.whatwg.org/entities.json), filtered to the
//! semicolon-terminated names -- CommonMark only recognizes those (a
//! non-semicolon form like `&copy` is only valid in raw HTML, and the
//! CommonMark spec deliberately does not recognize it when decoding an
//! entity reference; see the "Entity and numeric character references"
//! section of the spec). Keys omit the leading `&` and trailing `;`.
//! Regenerate by re-fetching that URL and re-running the generator
//! described in `inline.zig`'s entity-decoding doc comment if the table
//! ever needs updating.

const std = @import("std");

pub const table = std.StaticStringMap([]const u8).initComptime(.{
    .{ "AElig", "\u{c6}" },
    .{ "AMP", "&" },
    .{ "Aacute", "\u{c1}" },
    .{ "Abreve", "\u{102}" },
